...
# --- Core Pattern Matching and Prediction Logic (Adapted for DB) ---
...
def get_next_pattern_prediction(history: list) -> tuple[Optional[str], str]:
    """Analyzes history to find next prediction (Logic kept identical to original)."""
...
def generate_new_prediction(user_id: int, outcome: str) -> str:
    ...
    This function updates the user's history and prediction in the DB.
    """
    ...
    if outcome == 'win':
        if pattern_prediction:
            ...
            # Fallback to randomly alternating on a win reset
