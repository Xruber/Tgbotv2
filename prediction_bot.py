...
async def receive_utr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not plan_key or plan_key not in PREDICTION_PLANS:
        # If plan key is missing (likely due to a preceding error), inform the user
    ...
    try:
        ...
        # Log this error, but do not block the user flow
        logger.error(f"Failed to send admin alert to ADMIN_ID {ADMIN_ID}: {e}")
