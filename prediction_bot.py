...
def is_subscription_active(user_data) -> bool:
    ...
    # Note: For 'permanent' plan, expiry_timestamp will be a very large number, effectively always active.
...
async def select_plan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    query = update.callback_query
    await query.answer() # <--- This sends the "loading" acknowledgement to Telegram
    
    plan_key = query.data.replace("select_plan_", "") # <--- This should be '7_day'
    ...
    try:
        ...
        # 🚨 NEW: Log the error and tell the user there was a problem
        ...
        return ConversationHandler.END # End flow on fatal error
...
async def admin_accept_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # The 'permanent' plan should not schedule a removal job
...
def main() -> None:
    ...
    prediction_flow_handler = ConversationHandler(
        ...
        states={
            SELECTING_PLAN: [
                CallbackQueryHandler(select_plan, pattern="^select_plan_"), # <-- THIS HANDLES YOUR CLICK
            ],
            WAITING_FOR_PAYMENT_PROOF: [
