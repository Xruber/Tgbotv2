...
# --- Prediction & Payment Constants ---
REGISTER_LINK = "https://t.me/+pR0EE-BzatNjZjNl" # Replace with your group link
PAYMENT_IMAGE_URL = "https://cdn.discordapp.com/attachments/980672312225460287/1433268868255580262/Screenshot_20251029-1135273.png?ex=69041327&is=6902c1a7&hm=517ecf60348db21c748797801a6bcdc08336d7020d3bf41c889abe22ba4a4d26&" # ⚠️ REPLACE THIS
...
def get_user_data(user_id):
    ...
    # Ensure all keys are present for existing users
    if "current_level" not in user: user["current_level"] = 1
    if "current_prediction" not in user: user["current_prediction"] = random.choice(['Small', 'Big'])
    if "history" not in user: user["history"] = []
    if "current_pattern_name" not in user: user["current_pattern_name"] = "Random (No History)"
    if "last_game_id" not in user: user["last_game_id"] = '100'
...
def get_next_pattern_prediction(history: list) -> tuple[Optional[str], str]:
    """Analyzes history to find next prediction."""
    ...
    for pattern_list, pattern_name in ALL_PATTERNS:
        ...
        if recent_history_len < pattern_len:
            if recent_history == pattern_list[:recent_history_len]:
                best_match_prediction = pattern_list[recent_history_len]
...
def generate_new_prediction(user_id: int, outcome: str) -> str:
    """
    Generates the next prediction based on User's Rules:
    1. Win -> Repeat same result (Streak).
    2. 1st Loss -> Opposite result (Switch).
    3. 2nd+ Loss -> Run Pattern Analysis.
    """
    ...
    # If user clicked WIN, the actual outcome was what we predicted.
    # If user clicked LOSS, the actual outcome was the opposite.
    ...
    # Current level is retrieved from DB. 
    # NOTE: The level is incremented in 'handle_prediction_feedback' BEFORE this function is called.
    # So if we were at Level 1 and lost, current_level is now 2.
    current_level = state.get("current_level", 1)
    
    new_prediction = current_prediction # Default init
    pattern_name = "Analyzing..."

    if outcome == 'win':
        # RULE: If User Clicks WIN -> Continue with the same result
        new_prediction = current_prediction
        pattern_name = "Winning Streak (Repeat)"
        
    elif outcome == 'loss':
        # Check consecutive losses based on the current level
        
        if current_level == 2:
            # RULE: If User Clicks LOSS (1st time) -> Give Opposite
            # Level 2 means we just came from Level 1 and lost once.
            new_prediction = 'Small' if current_prediction == 'Big' else 'Big'
            pattern_name = "Switch (After 1 Loss)"
            
        elif current_level >= 3:
            # RULE: If User Clicks LOSS (2 times or more) -> Run Patterns
            pattern_prediction, found_pattern_name = get_next_pattern_prediction(history)
            
            if pattern_prediction:
                new_prediction = pattern_prediction
                pattern_name = found_pattern_name
            else:
                # If patterns don't find a match, default to random switch
                new_prediction = 'Small' if current_prediction == 'Big' else 'Big'
                pattern_name = "Random (No Pattern Found)"
        
        else:
            # Fallback for safe handling
            new_prediction = 'Small' if current_prediction == 'Big' else 'Big'
            pattern_name = "Standard Switch"
...
# --- Function to grant access (called by admin_accept) ---
...
async def receive_utr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    admin_keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✅ Accept Request", callback_data=f"admin_accept_{user_id}_{plan_key}"),
...
async def admin_accept_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if len(parts) < 4: raise ValueError("Callback data too short.")
...
async def receive_period_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if period_number < 1: raise ValueError
    except ValueError:
        await update.message.reply_text("⚠️ Invalid Period Number. Please send a valid positive number.")
        return WAITING_FOR_PERIOD_NUMBER
...
def main() -> None:
    ...
    # 4. Admin Approval Handler
    application.add_handler(CallbackQueryHandler(admin_accept_request, pattern="^admin_accept_"))
    application.add_handler(CallbackQueryHandler(admin_reject_request, pattern="^admin_reject_"))
