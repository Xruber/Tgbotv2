...
MAX_HISTORY_LENGTH = 4                  # How many past results to track for pattern matching
...
async def receive_utr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_data = get_user_data(user_id)
    # Get the plan key from the user's DB entry (stored in select_plan)
    ...
    # 3. 🔥 CRITICAL FIX: Explicitly call start() to refresh the main menu.
    #    This ensures the user sees the new menu with the 'Under Review' status immediately.
    #    We pass the original `update` object so the message is sent to the correct chat.
