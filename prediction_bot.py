...
ADMIN_ID = int(os.getenv("ADMIN_ID", "6239774927")) #  REPLACE THIS

# --- Prediction & Payment Constants (New/Updated) ---
REGISTER_LINK = "https://t.me/+pR0EE-BzatNjZjNl" # Replace with your group link
PAYMENT_IMAGE_URL = "https://cdn.discordapp.com/attachments/980672312225460287/1433268868255580262/Screenshot_20251029-1135273.png?ex=69041327&is=6902c1a7&hm=517ecf60348db21c748797801a6bcdc08336d7020d3bf41c889abe22ba4a4d26&" #  REPLACE THIS WITH YOUR QR/UPI IMAGE URL
...
# --- NEW PATTERN LOGIC CONSTANTS ---
# Loss sequence patterns to trigger a switch
LOSS_PATTERN_1 = ['Small', 'Big', 'Small']
LOSS_PATTERN_2 = ['Big', 'Small', 'Big']
PATTERN_LENGTH = 3
...
def get_user_data(user_id):
    ...
    if user is None:
        user = {
            ...
            "history": [], # History will now track the 'actual outcomes' of the last N games
...
# --- Core Pattern Matching and Prediction Logic (SIMPLIFIED/REVISED) ---
...
def generate_new_prediction(user_id: int, outcome: str) -> str:
    """
    Generates the next prediction based on the new simplified logic:
    1. WIN: Repeat the predicted outcome (i.e., switch the next prediction).
    2. LOSS: Repeat the previous prediction (Martingale trend).
    3. 3 Consecutive LOSSES: Check for 'Small, Big, Small' or 'Big, Small, Big' actual outcomes
       and switch prediction to the opposite of the last outcome.
    """
    ...
    # --- 1. Update History ---
    # History tracks the ACTUAL OUTCOMES of the last N games.
    ...
    new_prediction = current_prediction # Default: Keep the current prediction for loss (Martingale)
    pattern_name = "Martingale Trend" 

    # --- 2. Apply New Logic ---
    
    if outcome == 'win':
        # Case 1: WIN
        # The user's prediction was correct. The next prediction should be the opposite of the outcome,
        # which means the opposite of the prediction (as per "User clicks win continue with Big" which means 
        # the NEXT PREDICTION should be 'Big' if the user's PREDICTION was 'Small' and they won).
        # In simple terms: always alternate on a win.
        new_prediction = 'Big' if current_prediction == 'Small' else 'Small'
        pattern_name = "Alternating on Win"

    elif outcome == 'loss':
        # Case 2: LOSS (less than 3 in a row)
        # Default Martingale: Repeat the same bet as the last round (current_prediction).
        new_prediction = current_prediction
        pattern_name = "Martingale Trend"

        # Case 3: Check for 3 Consecutive LOSSES (which means 3 entries in history)
        if current_level >= PATTERN_LENGTH + 1: # We are on level 4 or higher (meaning 3 losses occurred)
...
# The rest of the code remains the same as the original, using the updated
# generate_new_prediction logic.
...
async def receive_period_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Check if the input is a number (or a string representing a number)
        # We don't need to enforce it being an integer but must ensure it's not a command.
        if not re.match(r'^\d+$', period_number_str):
             raise ValueError
        period_number = period_number_str

    except ValueError:
        await update.message.reply_text(" Invalid Period Number. Please send a valid number.")
        return WAITING_FOR_PERIOD_NUMBER
...
async def handle_prediction_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if feedback == "win":
        ...
        if current_level < MAX_LEVEL:
            ...
            new_level = MAX_LEVEL
            level_message = f" **MAX LEVEL REACHED ({MAX_LEVEL})**."
    ...
    # The new logic is fully contained within this function
    new_prediction = generate_new_prediction(user_id, feedback)
    
    # Reload user data to get the updated pattern name from the DB
