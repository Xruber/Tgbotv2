...
(
    ...
    WAITING_FOR_UTR,
    # 🆕 NEW STATE ADDED FOR THE PREDICTION REQUEST
    WAITING_FOR_PERIOD_NUMBER, 
) = range(4) # Changed range to 4
...
async def show_prediction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if is_subscription_active(user_data):
        ...
        # 🚨 FIX 2: Catch the BadRequest error if the keyboard content is the same.
        try:
            ...
            if "message is not modified" not in str(e).lower():
                raise # Re-raise other unexpected BadRequests
...
async def receive_period_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if period_number < 1: # Ensure it's a positive number
            raise ValueError
    except ValueError:
        await update.message.reply_text(
    ...
    # --- Prediction Logic ---
    # The pattern is: Small, Big, Big, Small (a cycle of 4 elements)
    # The index is calculated as: (period_number - 1) % len(PREDICTION_PATTERN)
    
    pattern_index = (period_number - 1) % len(PREDICTION_PATTERN)
...
def main() -> None:
    ...
    # 3. 🆕 NEW: Prediction REQUEST Conversation Handler
