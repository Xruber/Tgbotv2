...
def start_target_session(user_id, target_key, last_period_num):
    ...
    # --- FIX: Initialize V4 properly with History ---
    user_data = get_user_data(user_id)
    history = user_data.get("history", [])
    
    if history:
        last_pred = history[-1]
        # Simulate a 'win' to get the trend started
...
def process_target_outcome(user_id, outcome):
    ...
    # --- 2. Update Real History for V4 Accuracy ---
    ...
    # Convert index (0-4) to level (1-5) for the engine
