...
def process_target_outcome(user_id, outcome):
    ...
    # --- Fetch NEW Period (Crash Fix) ---
    game_type = session.get("game_type", "30s")
    next_period, _ = get_game_data(game_type)
    
    # SAFETY: If API returns None, manually increment the old period to prevent crash
    if not next_period:
        try:
            next_period = str(int(session["current_period"]) + 1)
        except:
            return session, "Ended" # Stop if completely broken

    # Ensure period actually moved forward
    if next_period == session["current_period"]:
         try:
            next_period = str(int(session["current_period"]) + 1)
         except:
            pass
