...
# ==========================================
# 1. CORE USER FUNCTIONS (Original + Wallet Update)
# ==========================================
...
def get_user_data(user_id):
    ...
    # Ensure Wallet Exists for Old Users
    if "wallet" not in user:
        default_wallet = {"balance": 0.0, "holdings": {}, "invested_amt": {}}
        users_collection.update_one({"user_id": user_id}, {"$set": {"wallet": default_wallet}})
...
INITIAL_TOKENS = [
    {"symbol": "TET", "name": "Texhet", "price": 10.0},
    {"symbol": "GLL", "name": "Gallium", "price": 5.5},
    {"symbol": "GGC", "name": "GigaCoin", "price": 100.0},
    {"symbol": "LKY", "name": "LOWKEY", "price": 0.5},
    {"symbol": "TSK", "name": "Tasket", "price": 12.0},
    {"symbol": "MKY", "name": "Milkyy", "price": 25.0},
    {"symbol": "HOA", "name": "Hainoka", "price": 8.0},
    {"symbol": "ZDR", "name": "Zendora", "price": 1.2},
    {"symbol": "FLX", "name": "Flux", "price": 45.0},
    {"symbol": "VRT", "name": "Vortex", "price": 150.0},
    {"symbol": "CRM", "name": "Crimson", "price": 7.0},
    {"symbol": "AER", "name": "Aether", "price": 90.0},
    {"symbol": "PLS", "name": "Pulse", "price": 3.3},
    {"symbol": "ION", "name": "Ion", "price": 18.0},
    {"symbol": "NVX", "name": "NovaX", "price": 60.0}
]

def init_tokens():
    """Initializes tokens in DB if they don't exist."""
...
def update_token_price(symbol, new_price):
    ...
    if tokens_collection is not None:
        tokens_collection.update_one({"symbol": symbol}, {"$set": {"price": float(new_price)}})
...
def get_user_wallet(user_id):
    """Returns user wallet object, ensuring it exists."""
    u = get_user_data(user_id)
    return u.get("wallet", {"balance": 0.0, "holdings": {}, "invested_amt": {}})

def update_wallet_balance(user_id, amount):
    """Adds/Subtracts fiat balance safely."""
    if users_collection is not None:
        users_collection.update_one(
            {"user_id": user_id}, 
            {"$inc": {"wallet.balance": float(amount)}}
        )

def update_token_holding(user_id, symbol, quantity, cost=0):
    """
    Updates token quantity.
    cost: Used to track total investment for ROI calculation.
    """
    if users_collection is not None:
        users_collection.update_one(
            {"user_id": user_id},
            {
                "$inc": {
                    f"wallet.holdings.{symbol}": quantity,
                    f"wallet.invested_amt.{symbol}": cost
                }
            }
        )
...
def trade_token(user_id, symbol, quantity, price, is_buy=True):
    """
    Handles Buying/Selling logic atomically.
    is_buy=True: Deduct Money, Add Token
    is_buy=False: Add Money, Deduct Token
    """
    if users_collection is None: return
    ...
    if is_buy:
        # Deduct Money, Add Token, Track Investment
        users_collection.update_one(
            {"user_id": user_id},
            {
                "$inc": {
...
# ==========================================
# 7. TRANSACTION HISTORY SYSTEM (NEW)
# ==========================================
...
def create_transaction(user_id, tx_type, amount, method, details):
    ...
    tx_data = {
        ...
        "amount": float(amount),
        "method": method,
        "details": details, # UTR or Bank Details
        "status": "pending",
        "timestamp": time.time()
    }
