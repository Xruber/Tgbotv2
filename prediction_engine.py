...
# --- V5 ENGINE LOGIC (MD5 + Salt) ---
def get_v5_logic(period_number):
    """
    Logic: MD5(Period + Salt) -> Last Char -> Dec -> % 10 -> Prediction
    """
    # 1. Combine Period + Salt (Suffix)
    ...
    salt_msg = " (Fixed)" if V5_SALT else ""
    pattern_name = f"V5 MD5{salt_msg} ({last_char}->{digit})"
