...
async def select_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer() # 1. Fix: Tells Telegram the button click was received
    
    # 2. Fix: Correctly extract keys like "7_day"
    key = query.data.replace("buy_", "")
    
    if key not in PREDICTION_PLANS:
        await query.edit_message_text("❌ Error: Plan not found in config.")
        return ConversationHandler.END

    context.user_data["plan"] = key
    plan_details = PREDICTION_PLANS[key]
    
    caption_text = (
        f"✅ **Selected:** {plan_details['name']} - {plan_details['price']}\n\n"
        "1. Pay using the QR Code / UPI.\n"
        "2. Click **'Sent 🟢'** below.\n"
        "3. You will be asked for the UTR."
    )
    ...
    try:
        ...
        # 3. Fix: Fallback if Image URL is broken/expired
        logger.error(f"Failed to send image: {e}")
        await context.bot.send_message(
            chat_id=query.from_user.id,
            text=f"⚠️ **Image Error:** The QR Code image link in `config.py` is invalid or expired.\n\n{caption_text}",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Sent 🟢", callback_data="sent")]])
        )
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Split the data: ['adm', 'ok', '12345', '7', 'day']
    data = query.data.split("_")
    action = data[1]
    uid = int(data[2])
    
    if action == "ok":
        # FIX: Join everything after the user ID to get the full plan name (e.g., "7_day")
        plan_key = "_".join(data[3:])
        
        # Double check if plan exists before granting
        if plan_key in PREDICTION_PLANS:
            await grant_prediction_access(uid, plan_key, context)
            await query.edit_message_text(f"✅ Access Granted to User {uid} for {PREDICTION_PLANS[plan_key]['name']}.")
        else:
             await query.edit_message_text(f"⚠️ Error: Plan '{plan_key}' not found.")
            
    else:
        # Reject logic
        try:
            await context.bot.send_message(uid, "❌ **Payment Verification Failed.**\nYour request was rejected by the admin.")
        except:
            pass # User might have blocked bot
            
        await query.edit_message_text(f"❌ Request for User {uid} Rejected.")
