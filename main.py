...
async def start_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if item_key in PREDICTION_PLANS:
        ...
        # Fallback for main menu "Buy Strategy"
...
async def confirm_sent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """FIX: Dedicated function to handle the button click properly."""
    query = update.callback_query
    await query.answer()
    
    # Edit the message to ask for UTR
    try:
...
def main():
    ...
    app.add_handler(CommandHandler("cancel", cancel)) # FIX: Global Cancel
    ...
    # 5. Purchase Loop (Shared) - FIXED Reentry
    buy_h = ConversationHandler(
        ...
        states={
            ...
            WAITING_FOR_PAYMENT_PROOF: [CallbackQueryHandler(confirm_sent, pattern="^sent$")], # Fixed here
