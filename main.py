...
async def shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "shop_target":
        ...
        # Back to main shop
        await packs_command(query.message, context) # Re-use command logic but edit text logic needed if separate
        # Simplified: just trigger command logic logic again or edit
        keyboard = [
            [InlineKeyboardButton("🎯 Target Prediction Packs", callback_data="shop_target")],
            [InlineKeyboardButton(f"🔢 Number Shot Pack ({NUMBER_SHOT_PRICE})", callback_data=f"buy_{NUMBER_SHOT_KEY}")]
        ]
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "ok":
        ...
        await context.bot.send_message(uid, "❌ **Payment Rejected.** Check UTR and try again.")
...
def main():
    ...
    # 1. Main Commands
    app.add_handler(CommandHandler("start", lambda u,c: start_buy(u,c) if 0 else u.message.reply_text("Use /start"))) # Simplified start
    ...
    # Need to register standard start properly (reused code from previous msg for start_command)
    from main import start_command # Self-import fix or just define above
