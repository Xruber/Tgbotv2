...
# FIX: Added calculate_sequence to imports
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "I am prediction bot with 5-6 levels i can guarantee profit💎.",
...
async def target_resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not session or not session.get("is_active"):
        await query.edit_message_text("❌ No active session found to resume.")
        return ConversationHandler.END

    # FIX: Patch old sessions that don't have the new 'sequence' key
...
async def display_target_step(query, session, status_text):
    ...
    seq = session.get('sequence', TARGET_SEQUENCE) # Fallback to default if somehow still missing
