...
async def switch_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{'✅ ' if curr=='V3' else ''}V3: Analyzed Streak", callback_data="set_mode_V3")]
    ])
...
async def display_target_step(query, session, status_text):
    ...
    # FIX: Get bet from the stored sequence
    bet = session['sequence'][lvl_idx]
