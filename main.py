...
from handlers_shop import (
    packs_command, shop_callback, start_buy, confirm_sent, 
    receive_utr, admin_action, target_command, target_resume, 
    start_target_game, target_loop
)
from handlers_sureshot import (
    sureshot_command, sureshot_start, sureshot_refresh, sureshot_outcome
)
...
from handlers_wallet import (
    wallet_command, tokens_command, buy_token_callback, 
    sell_menu, sell_token_callback, admin_payment_handler,
...
async def start_command(update: Update, context, edit_mode=False):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💰 My Wallet", callback_data="shop_wallet"), InlineKeyboardButton("📈 Invest Tokens", callback_data="wallet_invest")],
        [InlineKeyboardButton("🎯 Target Mode", callback_data="shop_target"), InlineKeyboardButton("🛒 VIP Shop", callback_data="shop_main")],
        [InlineKeyboardButton("👤 My Profile", callback_data="my_stats"), InlineKeyboardButton("🎁 Redeem Code", callback_data="btn_redeem_hint")],
        [InlineKeyboardButton("💬 Support / CC", url="https://t.me/your_support_handle")]
    ])
...
def main():
    ...
    dep_conv = ConversationHandler(
        ...
        fallbacks=[CallbackQueryHandler(wallet_command, pattern="^shop_wallet$")],
        per_user=True
    )
    ...
    # 4. OTHER CALLBACKS (Must be last to avoid catching conversation patterns)
    ...
    app.add_handler(CallbackQueryHandler(wallet_command, pattern="^shop_wallet$"))
    app.add_handler(CallbackQueryHandler(tokens_command, pattern="^wallet_invest$"))
    app.add_handler(CallbackQueryHandler(buy_token_callback, pattern="^buy_token_"))
    app.add_handler(CallbackQueryHandler(sell_menu, pattern="^wallet_sell$"))
    app.add_handler(CallbackQueryHandler(sell_token_callback, pattern="^sell_token_"))

    print("--------------------------------------------------")
    print(f"✅ Bot Online (Wallet + Tokens + Game v2)")
