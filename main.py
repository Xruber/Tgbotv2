...
from prediction_engine import process_prediction_request, get_bet_unit, get_number_for_outcome, get_v5_logic # ADDED IMPORT
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Main Menu. Handles Referral Tracking."""
...
async def receive_period_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- V5 LOGIC OVERRIDE ---
    # If mode is V5, we calculate prediction instantly based on the period
    if mode == "V5":
        pred, pat, v5_digit = get_v5_logic(period)
        # Update DB so handle_feedback knows what we predicted
        update_user_field(user_id, "current_prediction", pred)
        update_user_field(user_id, "current_pattern_name", pat)
        
        # Force show the V5 number if user has number shot
        if user_data.get("has_number_shot"):
            shot_num = v5_digit 
        else:
            shot_num = None # Or random if you prefer default behavior
    else:
        # Standard modes read from DB (pre-calculated)
...
async def handle_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Generate NEXT prediction (Note: V5 ignores this and recalculates on next period input)
    ...
    number_line = ""
    # Only show number for standard modes here. V5 waits for period input.
    if user_data.get("has_number_shot") and mode != "V5":
        shot_num = get_number_for_outcome(new_pred)
        number_line = f"Number: **{shot_num}**\n"

    # For V5, we don't show specific prediction in the feedback result, 
    # because it depends on the *next* period number which we don't know yet.
    if mode == "V5":
        next_pred_text = "Wait for Period..."
        pat = "V5 Logic"
    else:
        next_pred_text = new_pred

    await query.edit_message_text(
        f"Result: **{outcome.upper()}**\n\n➡️ **Next Prediction ({mode})**\n"
        f"Bet: **{next_pred_text}**\n{number_line}Level: {new_lvl} ({unit} Units)\nReason: {pat}\n\n{PREDICTION_PROMPT}",
        parse_mode="Markdown"
    )
    return WAITING_FOR_PERIOD_NUMBER
